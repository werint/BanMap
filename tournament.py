        ...
        async def callback(interaction):
            ...
            if len(self.data['banned']) >= 4:
                ...
                embed = discord.Embed(
                    color=0xFA747D,
                    title="Бан карт",
                    description=f"Осталось банов: {4 - len(self.data['banned'])}\nЗабанено: {', '.join(self.data['banned']) if self.data['banned'] else 'нет'}"
                )
...
async def startmatch(interaction, captain1: discord.Member, captain2: discord.Member):
    ...
    embed = discord.Embed(color=0xFA747D, title="Бан карт", description=f"Осталось банов: 4")
    embed.add_field(name="Капитан 1", value=captain1.mention)
    embed.add_field(name="Капитан 2", value=captain2.mention)
    embed.set_footer(text=f"Ход: {current.display_name}")
